...
# pdf("fig/test.pdf", width = 4, height = 7, bg = "white")
source("script/00_functions/fun_MT.R")
...
par(mfrow = c(3,3), mar = c(2,2,2,2), oma = c(4,4,0,0))
...
# regression to find T0
par(mfrow = c(3,3), mar = c(2,2,2,2), oma = c(4,4,0,0))
lm_night_ler <- vector("list", length = length(levels(ag_lenviro$device)))
for(i in seq_along(levels(ag_lenviro$device))){
  name <- levels(ag_lenviro$device)[[i]]
  data <- ag_lenviro %>%
    filter(!daytime) %>%
    filter(device == name)
  plot(ler ~ temp, data = data, xlim = c(22,27.2), ylim = c(0,7.2))
  lm_night_ler[[i]] <- FunRegressionPlot(x = data$temp, y = data$ler, edge = c(0.6,0.9))
  #
  # lm_night_ler[[i]] <- lm(ler ~ temp, data = data)
  #
  # plot(ler ~ temp, data = data)
  # abline(lm_night_ler[[i]])
}
mtext("temp", side = 1, outer = T, line = 2)
mtext("ler", side = 2, outer = T, line = 2)
