...
my_path <- list.files(path = "data/01_lenviro", recursive = T, pattern = "\\.txt$")
lenviro <- vector("list", length(my_path))
for (i in seq_along(my_path)) {
  # collect file info and write experiment_label
  file_info <- unlist(str_split(my_path[i], pattern = "(/|-|_|\\.)"))
  experiment_label <- str_c(file_info[4], file_info[2], file_info[3], "_", file_info[length(file_info)-1])
  # read in as single string
  file <- read_file(str_c("data/01_lenviro/", my_path[i])) %>%
...
# pdf("fig/setup_experiment_var_all_devices.pdf", width = 4, height = 7, bg = "white")
# fun_plot_var_ex(df = lenviro)
# dev.off()
...
# pdf("fig/setup_experiment_ler_model_all_devices.pdf", width = 4, height = 7, bg = "white")
# mod_summary <- fun_plot_lerModel_ex(lenviro)
# dev.off()
...
# pdf("fig/setup_experiment_var_all_devices_ag.pdf", width = 4, height = 7, bg = "white")
fun_plot_var_ex(df = ag_lenviro)
# dev.off()
...
# pdf("fig/setup_experiment_ler_model_all_devices_ag.pdf", width = 4, height = 7, bg = "white")
mod_summary <- fun_plot_lerModel_ex(ag_lenviro)
# dev.off()
