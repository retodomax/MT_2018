...
mod_summary <- fun_plot_lerModel(lenviro)
# dev.off()
...
# what it the goal of spectrum()?
# 
spectrum(mod_summary$device11$residuals)
plot(mod_summary$device11$residuals)
ssp <- spectrum(df$residuals)

per <- 1/ssp$freq[ssp$spec==max(ssp$spec)]
reslm <- lm(df$residuals ~ +cos(2*pi/per*df$cTime))
summary(reslm)

# 6) go further with chapter "leaf elongation rate per termal time"
# 7) use new data from Reckenholz (apply evaluation to all data sets)
# => new code for device (combination of Chamber (A,B,C,D), Device number (1-16) and date (2017-03-01))
# 8) create variable "experiment (e.g. A3_20170301) which connects "lenviro" with a second table "design"
